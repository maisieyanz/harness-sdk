...
describe('MemoryManager', () => {
  ...
  describe('initAgent', () => {
    ...
    describe('store plugins', () => {
      ...
      // The PluginRegistry registers tools only for the plugins it holds, and it never holds a store
      // plugin, so the manager must surface those tools through its own getTools to reach the agent.
      it("registers a store plugin's tools alongside the store's own", () => {
        const plugin = createNamedPlugin('store:nav', [createNamedTool('from_plugin')])
        const mm = new MemoryManager({
          stores: [createMockStore('a', { plugins: [plugin], tools: [createNamedTool('from_store')] })],
        })

        expect(mm.getTools().map((registered) => registered.name)).toStrictEqual([
          'search_memory',
          'from_store',
          'from_plugin',
        ])
